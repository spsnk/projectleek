...
function newTileEngine(){
	var TileEngine = { //main canvas and demo container
		...
		createTiles: function(tilesArray, physicsArray) { //load tile array
			...
			function getX(index){return parseInt(TileEngine.tileWidth * parseInt(index % parseInt(TileEngine.tilesWide)))}
			function getY(index){return parseInt(TileEngine.tileHeight * parseInt(index / parseInt(TileEngine.tilesWide)))}
...
function newTile(){
	var Tile = {
		...
		init: function(x, y, width, height, source_index, physics_id){ //initialize sprite
			...
			//Tile.baseSourceIndex = source_index; // set index of tile source for this tile
...
function newTileSource(){ //image used to create tile 
	var TileSource = {
		...
		init: function(width, height, src_x, src_y, source){
			...
			//console.log('x:'+src_x+' y:'+src_y);
			//console.log(TileSource.sourceImage.image);
